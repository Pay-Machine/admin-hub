-- Fixed security hardening for profiles table
...
-- 3. Create a separate, limited superadmin policy for emergency access only (business hours)
...
-- Separate policy for superadmin updates with recent audit trail requirement
CREATE POLICY "Superadmin can update with recent audit" 
ON public.profiles 
FOR UPDATE 
TO authenticated
USING (
...
-- 6. Add logging for api_tokens access (INSERT/UPDATE/DELETE only)
...
AS $$
BEGIN
  -- Log token access attempts for INSERT, UPDATE, DELETE
  INSERT INTO public.profile_access_log (
    ...
    auth.uid(),
    CASE 
      WHEN TG_OP = 'INSERT' THEN 'token_created'
      WHEN TG_OP = 'UPDATE' THEN 'token_updated' 
      WHEN TG_OP = 'DELETE' THEN 'token_deleted'
    END,
    NOW()
  );
...
-- Create trigger for token access logging (only for modification operations)
